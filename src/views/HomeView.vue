...
const services = computed(() => [
  {
    ...
    desc: t('services.items.checkup.description'),
  ...
  },
  {
    ...
    desc: t('services.items.referral.description'),
  ...
  },
  {
    ...
    desc: t('services.items.stemCell.description'),
...
const pharmacyItems = computed(() => [
  {
    ...
    cat: t('pharmacy.categories.painRelief'),
  ...
  },
  {
    ...
    cat: t('pharmacy.categories.cardiovascular'),
  ...
  },
  {
    ...
    cat: t('pharmacy.categories.respiratory'),
    tag: t('pharmacy.tags.otc'),
    type: 'otc',
    icon: 'ph-first-aid',
  },
  {
    ...
    cat: t('pharmacy.categories.regenerative'),
...
<template>
  <main class="flex-1">
    <div
      class="relative w-full flex items-center justify-center overflow-hidden border-b border-border bg-background pt-32 pb-20 md:pt-40 md:pb-32 lg:pt-48 lg:pb-40"
    >
      ...
      <Container class="relative z-10 text-center px-6">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <Button size="lg" class="px-8 h-12 rounded-xl shadow-lg">{{
            t('hero.btnOverseas')
          }}</Button>
          <Button
            variant="outline"
            size="lg"
            class="px-8 h-12 rounded-xl bg-background/50 backdrop-blur-sm shadow-sm"
            >{{ t('hero.btnLocal') }}</Button
          >
        </div>
      </Container>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="ml-4">
            <CardTitle
              class="text-base"
              :class="item.variant === 'highlight' ? 'text-white' : ''"
              >{{ t(item.titleKey) }}</CardTitle
            >
            <CardDescription
              class="text-xs"
              :class="item.variant === 'highlight' ? 'text-white/80' : ''"
              >{{ t(item.descKey) }}</CardDescription
            >
          </div>
        </Card>
      </div>
        ...
        <div class="mobile-swipe-row lg:grid-cols-3">
          ...
          >
            ...
            <div class="absolute bottom-0 p-6 text-white w-full">
              ...
              <Button size="sm" variant="secondary" class="rounded-full font-bold px-6 h-9">{{
                t('common.viewDetail')
              }}</Button>
            </div>
          </Card>
    ...
    <section id="pharmacy" class="py-16 md:py-24 bg-background">
      <Container>
        <div class="flex items-center justify-between mb-10 px-4 md:px-0">
          ...
          <Button variant="ghost" class="text-primary p-0 h-10 px-4">{{
            t('pharmacy.viewAll')
          }}</Button>
        </div>
        ...
        <div class="mobile-swipe-row sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            <div
              class="relative aspect-square bg-white rounded-xl flex items-center justify-center mb-4 border border-border/30 shadow-sm"
            >
              <span :class="cn(pharmacyTagVariants({ type: item.type as any }))">{{
                item.tag
              }}</span>
    ...
    <section id="information" class="py-16 md:py-24 bg-muted/20 border-t border-border">
      <Container>
        ...
        <div class="mobile-swipe-row lg:grid-cols-2">
          ...
          >
            <div
              class="w-24 md:w-32 bg-primary/5 border-r border-border flex items-center justify-center shrink-0"
            >
              <i :class="['ph text-4xl text-primary/40', caseItem.icon]"></i>
            </div>
            <div class="p-4 md:p-6 flex flex-col justify-center overflow-hidden">
              <span
                class="inline-flex w-fit rounded-md border border-border bg-background px-2 py-0.5 text-[10px] font-bold mb-2"
                >{{ caseItem.tag }}</span
              >
    ...
    <section class="py-16 md:py-24 border-t border-border bg-white">
      <Container>
        ...
        <div class="mobile-swipe-row md:grid-cols-4 lg:grid-cols-5">
          ...
          >
            <div
              class="w-20 h-20 mx-auto rounded-full p-1 border-2 border-primary/10 group-active:border-primary transition-all mb-3 overflow-hidden shadow-inner"
            >
    ...
    <section id="partners" class="py-16 bg-slate-50 border-t border-border">
      <Container>
        <h3
          class="text-[10px] font-bold tracking-widest text-muted-foreground uppercase mb-8 text-center"
        >
          {{ t('partners.title') }}
        </h3>
        <div
          class="flex flex-wrap justify-center items-center gap-6 md:gap-16 opacity-60 grayscale hover:grayscale-0 transition-all duration-500"
        >
          <div
            v-for="(partner, index) in partners"
            :key="index"
            class="flex items-center gap-2 font-black text-sm md:text-base text-slate-700"
          >
