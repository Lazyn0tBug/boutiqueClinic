...
const caseStudies = computed<CaseStudy[]>(() => [
  {
    ...
    summary: t('cases.items.cancer.summary'),
    icon: 'ph-shield-check',
  },
  {
    tag: t('cases.items.lungCancer.tag'),
    title: t('cases.items.lungCancer.title'),
    summary: t('cases.items.lungCancer.summary'),
    icon: 'ph-users-three',
  },
])
...
const doctors = computed<Doctor[]>(() => [
  ...
  },
  {
    ...
    title: t('doctors.items.sato.title'),
...
<template>
  <main class="flex-1 scroll-container">
    <div class="relative w-full min-h-[70dvh] md:min-h-[85dvh] flex items-center justify-center overflow-hidden border-b border-border bg-background">
      ...
      <Container as="header" size="lg" class="relative z-10 w-full text-center py-20 md:py-32 page-container">
    ...
    <Container as="section" aria-label="快捷入口" class="-mt-4 relative z-20 sm:-mt-8 lg:-mt-12 mb-16">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 card-container">
        <Card class="flex flex-row items-center p-4 md:p-6 hover:border-primary/50 transition-colors">
          <div class="h-12 w-12 rounded-lg bg-primary/10 flex items-center justify-center text-primary border border-primary/20 shrink-0">
            <i class="ph ph-stethoscope text-2xl"></i>
          </div>
          <div class="ml-4">
            <CardTitle class="text-base">{{ t('quickAccess.consultation.title') }}</CardTitle>
            <CardDescription class="mt-1">{{ t('quickAccess.consultation.description') }}</CardDescription>
          </div>
        </Card>
        <Card class="flex flex-row items-center p-4 md:p-6 hover:border-primary/50 transition-colors">
          <div class="h-12 w-12 rounded-lg bg-secondary flex items-center justify-center text-secondary-foreground border border-border shrink-0">
            <i class="ph ph-heartbeat text-2xl"></i>
          </div>
          <div class="ml-4">
            <CardTitle class="text-base">{{ t('quickAccess.checkup.title') }}</CardTitle>
        ...
        <Card class="flex flex-row items-center p-4 md:p-6 bg-primary text-primary-foreground border-primary">
          <div class="h-12 w-12 rounded-lg bg-white/20 flex items-center justify-center text-white border border-white/20 shrink-0">
            <i class="ph ph-phone-call text-2xl"></i>
          </div>
          <div class="ml-4">
            <CardTitle class="text-base text-primary-foreground">{{ t('quickAccess.emergency.title') }}</CardTitle>
            <CardDescription class="mt-1 text-primary-foreground/80">{{ t('quickAccess.emergency.description') }}</CardDescription>
          </div>
        </Card>
      </div>
    </Container>
    ...
    <section id="services" class="py-16 md:py-24 bg-muted/30 border-y border-border">
      <Container class="page-container">
        <div class="flex flex-col items-center justify-center text-center mb-12">
          <h2 class="page-heading-2 font-bold tracking-tight text-foreground text-balance">
            {{ t('services.title') }}
          </h2>
        ...
        <div class="flex md:grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8 overflow-x-auto md:overflow-visible snap-x snap-mandatory scroll-card-container -mx-4 px-4 md:mx-0 md:px-0 pb-6 md:pb-0 scroll-hidden">
          <Card
            v-for="(service, index) in services"
            :key="index"
            class="overflow-hidden group flex flex-col hover:border-primary/40 transition-colors shrink-0 snap-center scroll-card-services md:w-auto"
          >
    ...
    <section id="pharmacy" class="py-16 md:py-24 bg-background">
      <Container class="page-container">
        ...
        <div class="flex md:grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 overflow-x-auto md:overflow-visible snap-x snap-mandatory scroll-card-container -mx-4 px-4 md:mx-0 md:px-0 pb-6 md:pb-0 scroll-hidden">
          <Card
            v-for="(medicine, index) in pharmacyItems"
            :key="index"
            class="p-4 md:p-5 group cursor-pointer hover:border-primary/40 transition-colors shrink-0 snap-center scroll-card-pharmacy md:w-auto"
          >
            <div class="aspect-square bg-muted rounded-md flex items-center justify-center relative mb-3 md:mb-4 border border-border">
              <span class="absolute top-2 left-2 inline-flex items-center rounded-full border px-2 md:px-2.5 py-0.5 text-xs font-semibold transition-colors" :class="medicine.tagClass">
                {{ medicine.tag }}
              </span>
            ...
            <CardDescription class="mt-1 line-clamp-2 text-xs md:text-sm md:min-h-[2.5rem]">
    ...
    <section id="information" class="py-16 md:py-24 bg-muted/30 border-t border-border">
      <Container class="page-container">
        <div class="mb-24">
          ...
          <div class="flex md:grid lg:grid-cols-2 gap-4 md:gap-6 overflow-x-auto md:overflow-visible snap-x snap-mandatory scroll-card-container -mx-4 px-4 md:mx-0 md:px-0 pb-6 md:pb-0 scroll-hidden">
            <Card
              v-for="(caseItem, index) in caseStudies"
              :key="index"
              class="flex flex-row md:flex-col lg:flex-row overflow-hidden hover:border-primary/50 transition-colors shrink-0 snap-center scroll-card-case md:w-auto"
            >
              <div class="w-1/3 md:w-full lg:w-1/3 bg-muted border-r md:border-r-0 md:border-b lg:border-r lg:border-b-0 border-border flex items-center justify-center p-4 md:p-8 aspect-square md:aspect-[4/3] lg:aspect-auto">
              ...
              <div class="w-2/3 md:w-full lg:w-2/3 p-4 md:p-6 flex flex-col justify-center">
                <span class="inline-flex w-fit items-center rounded-md border border-border bg-background px-2 py-0.5 text-xs font-semibold text-foreground mb-2 md:mb-3">{{ caseItem.tag }}</span>
                <CardTitle class="text-base md:text-lg mb-1 md:mb-2">{{ caseItem.title }}</CardTitle>
                <CardDescription class="line-clamp-2 text-xs md:text-sm mb-0 md:mb-4">
                  {{ caseItem.summary }}
                </CardDescription>
                <a href="#" class="text-xs md:text-sm font-medium text-primary hover:underline flex items-center gap-1 mt-2 md:mt-auto w-fit md:min-h-[44px]">
        ...
        <div>
          ...
          <div class="flex md:grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 overflow-x-auto md:overflow-visible snap-x snap-mandatory scroll-card-container -mx-4 px-4 md:mx-0 md:px-0 pb-6 md:pb-0 scroll-hidden">
            <Card
              ...
              :key="index"
              class="p-4 md:p-6 text-center group shrink-0 snap-center scroll-card-doctor md:w-auto"
            >
            ...
            <Card class="p-4 md:p-6 text-center flex flex-col items-center justify-center border-dashed border-2 hover:border-primary/50 transition-colors bg-transparent shadow-none cursor-pointer shrink-0 snap-center scroll-card-doctor md:w-auto min-h-[160px] md:min-h-[200px]">
      ...
      <Container class="page-container">
        ...
        <div class="flex flex-wrap justify-center items-center gap-6 md:gap-16 opacity-70 grayscale hover:grayscale-0 transition-all duration-500">
          <div
            ...
            :key="index"
            class="flex items-center gap-2 font-bold text-base md:text-lg text-foreground"
          >
